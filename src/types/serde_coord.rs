...
pub mod opt {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        deserializer.deserialize_tuple(2, visitor).or(Ok(None))
    }
}
...
impl<'de> serde::de::Visitor<'de> for CoordVisitor {
    ...
    {
        use cetkaik_core::absolute::{Column, Coord, Row};
        let mut column = None;
        let mut row = None;
        ...
        for i in 0..2 {
            if let Some(item) = visitor.next_element()? {
                ...
                return Err(serde::de::Error::invalid_length(i, &"2"));
            }
        }
        ...
        if let Some(column) = column {
            if let Some(row) = row {
                ...
                Err(serde::de::Error::missing_field("row"))
            }
        } else {
            Err(serde::de::Error::missing_field("column"))
        }
    }
}
...
{
    let visitor = CoordVisitor;
    deserializer.deserialize_tuple(2, visitor).and_then(|opt| {
        opt.ok_or_else(|| serde::de::Error::invalid_value(Unexpected::Other("null"), &"[str,str]"))
    })
}
